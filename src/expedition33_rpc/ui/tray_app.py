import base64
import contextlib
import io
import os
import sys
import threading
import time
from typing import Any

import pystray
from PIL import Image
from pystray import Menu, MenuItem

from expedition33_rpc.core import settings, startup
from expedition33_rpc.detection.detector import GameDetector, GameState
from expedition33_rpc.integrations import bridge_installer
from expedition33_rpc.integrations.discord_rpc import DiscordRPCManager
from expedition33_rpc.integrations.updater import AppUpdater, UpdateInfo, UpdateState

EMBEDDED_ICON_B64 = ""


def get_resource_path(filename: str) -> str:
    """Resolves resource path across dev environments and PyInstaller bundles."""
    if getattr(sys, "frozen", False):
        base_meipass = getattr(sys, "_MEIPASS", None)
        if base_meipass:
            bundled_pkg = os.path.join(base_meipass, "expedition33_rpc", "assets", filename)
            if os.path.exists(bundled_pkg):
                return bundled_pkg
            flat_bundled = os.path.join(base_meipass, filename)
            if os.path.exists(flat_bundled):
                return flat_bundled

        exe_dir = os.path.dirname(sys.executable)
        if os.path.exists(os.path.join(exe_dir, filename)):
            return os.path.join(exe_dir, filename)

    module_dir = os.path.dirname(os.path.abspath(__file__))
    pkg_asset = os.path.join(module_dir, "assets", filename)
    if os.path.exists(pkg_asset):
        return pkg_asset
    candidate_parent = os.path.join(module_dir, "..", "assets", filename)
    if os.path.exists(candidate_parent):
        return candidate_parent

    return filename


class ExpeditionTrayApp:
    def __init__(self, script_dir: str = ""):
        self.script_dir = script_dir

        self.detector = GameDetector()
        self.rpc_manager = DiscordRPCManager()

        self.current_state = GameState()
        self.running = True
        self.icon = None
        self.worker_thread = None
        self.bridge_auto_installed = False
        self._last_menu_state = None

        self.updater = AppUpdater(on_state_change=self.on_updater_state_change)

        # Attempt automatic combat bridge installation on startup if game folder exists
        self.check_and_auto_install_bridge()

    def on_updater_state_change(self):
        if self.icon is not None:
            with contextlib.suppress(Exception):
                self.icon.update_menu()

    def on_update_check_complete(self, update_info: UpdateInfo | None):
        if update_info and self.icon is not None:
            with contextlib.suppress(Exception):
                self.icon.notify(
                    f"Version {update_info.tag_name} is available. Open the tray menu to install.",
                    "Expedition 33 RPC Update",
                )
        self.on_updater_state_change()

    def cleanup_for_update(self):
        self.running = False
        self.rpc_manager.disconnect()
        if self.icon is not None:
            with contextlib.suppress(Exception):
                self.icon.stop()

    def check_and_auto_install_bridge(self):
        try:
            game_dir = bridge_installer.find_game_win64_directory()
            if game_dir and not bridge_installer.is_bridge_installed(game_dir):
                success, msg = bridge_installer.install_bridge(game_dir)
                if success:
                    self.bridge_auto_installed = True
                    print(f"[TrayApp] Automatically deployed Combat Bridge to: {game_dir}")
        except Exception as e:
            print(f"[TrayApp] Bridge auto-install check failed: {e}")

    def get_icon_image(self) -> Image.Image:
        icon_path = get_resource_path("icon.png")
        if os.path.exists(icon_path):
            try:
                return Image.open(icon_path).convert("RGBA")
            except Exception as e:
                print(f"[TrayApp] Warning loading icon file: {e}")

        # Infallible fallback: load embedded official 33 icon from base64
        try:
            raw = base64.b64decode(EMBEDDED_ICON_B64)
            return Image.open(io.BytesIO(raw)).convert("RGBA")
        except Exception as e:
            print(f"[TrayApp] Fallback icon error: {e}")
            return Image.new("RGBA", (64, 64), color=(212, 175, 55, 255))

    def update_loop(self):
        while self.running:
            try:
                anti_spoiler = settings.is_anti_spoiler_enabled()
                state = self.detector.get_game_state()
                self.current_state = state
                self.rpc_manager.update(state, anti_spoiler=anti_spoiler)

                # Periodic auto-install check while game is running
                if (
                    state.is_running
                    and not self.bridge_auto_installed
                    and not bridge_installer.is_bridge_installed()
                ):
                    self.check_and_auto_install_bridge()

                if self.icon is not None:
                    # Sync context menu when game status changes
                    menu_state = (
                        state.is_running,
                        state.process_pid,
                        state.zone_name,
                        state.in_combat,
                        state.enemy_name,
                        state.tower_stage_trial,
                        state.checkpoint_name,
                        self.rpc_manager.is_connected,
                        anti_spoiler,
                    )
                    if menu_state != self._last_menu_state:
                        self._last_menu_state = menu_state
                        with contextlib.suppress(Exception):
                            self.icon.update_menu()

                    if state.is_running:
                        if anti_spoiler:
                            mode = "In Combat" if state.in_combat else "Exploring"
                            self.icon.title = f"Expedition 33: [Protected] ({mode})"
                        else:
                            if state.in_combat and state.enemy_name:
                                mode = f"Fighting: {state.enemy_name}"
                            else:
                                mode = "In Combat" if state.in_combat else "Exploring"

                            loc = state.zone_name
                            if state.tower_stage_trial:
                                if isinstance(state.tower_stage_trial, tuple):
                                    stg, trl = state.tower_stage_trial
                                    is_it = state.game_language.startswith("it")
                                    trial_str = (
                                        f"Fase {stg}, Prova {trl}"
                                        if is_it
                                        else f"Stage {stg}, Trial {trl}"
                                    )
                                else:
                                    trial_str = str(state.tower_stage_trial)
                                loc = f"{loc} ({trial_str})"
                            elif state.tower_floor:
                                is_it = state.game_language.startswith("it")
                                floor_label = (
                                    f"Piano {state.tower_floor}"
                                    if is_it
                                    else f"Floor {state.tower_floor}"
                                )
                                if "piano" not in loc.lower() and "floor" not in loc.lower():
                                    loc = f"{loc} ({floor_label})"
                            elif state.checkpoint_name:
                                loc = f"{loc} ({state.checkpoint_name})"
                            self.icon.title = f"Expedition 33: {loc} ({mode})"
                    else:
                        self.icon.title = "Expedition 33 RPC (Waiting for game...)"
            except Exception as e:
                print(f"[TrayApp] Worker loop error: {e}")

            time.sleep(2)

    def menu_game_status(self, *_) -> str:
        if self.current_state.is_running:
            return f"Game: Running (PID: {self.current_state.process_pid})"
        return "Game: Not running"

    def menu_zone_status(self, *_) -> str:
        if not self.current_state.is_running:
            return "Location: --"
        if settings.is_anti_spoiler_enabled():
            return "Location: [Hidden - Anti-Spoiler]"
        zone_str = self.current_state.zone_name
        if self.current_state.tower_stage_trial:
            if isinstance(self.current_state.tower_stage_trial, tuple):
                stg, trl = self.current_state.tower_stage_trial
                is_it = self.current_state.game_language.startswith("it")
                trial_label = f"Fase {stg}, Prova {trl}" if is_it else f"Stage {stg}, Trial {trl}"
            else:
                trial_label = str(self.current_state.tower_stage_trial)
            zone_str = f"{zone_str} ({trial_label})"
        elif self.current_state.tower_floor:
            is_it = self.current_state.game_language.startswith("it")
            floor_label = (
                f"Piano {self.current_state.tower_floor}"
                if is_it
                else f"Floor {self.current_state.tower_floor}"
            )
            if "piano" not in zone_str.lower() and "floor" not in zone_str.lower():
                zone_str = f"{zone_str} ({floor_label})"
        elif self.current_state.checkpoint_name:
            zone_str = f"{zone_str} ({self.current_state.checkpoint_name})"
        return f"Location: {zone_str}"

    def menu_combat_status(self, *_) -> str:
        if not self.current_state.is_running:
            return "Status: --"
        anti_spoiler = settings.is_anti_spoiler_enabled()
        if self.current_state.in_combat:
            if not anti_spoiler and self.current_state.enemy_name:
                return f"Combat: Fighting {self.current_state.enemy_name}"
            return "Status: In Combat"
        return "Status: Exploring"

    def menu_discord_status(self, *_) -> str:
        if self.rpc_manager.is_connected:
            return "Discord: Connected"
        return "Discord: Standby (Listening)"

    def menu_anti_spoiler_toggle(self, *_) -> str:
        if settings.is_anti_spoiler_enabled():
            return "Anti-Spoiler Mode  [Enabled]"
        return "Anti-Spoiler Mode  [Disabled]"

    def action_toggle_anti_spoiler(self, *_):
        new_val = settings.toggle_anti_spoiler()
        print(f"[TrayApp] Anti-Spoiler toggled to: {new_val}")
        self.rpc_manager.last_update_state = None
        self.rpc_manager.update(self.current_state, anti_spoiler=new_val)
        if self.icon is not None:
            with contextlib.suppress(Exception):
                self.icon.update_menu()

    def menu_bridge_toggle(self, *_) -> str:
        if bridge_installer.is_bridge_installed():
            return "Combat Bridge  [Installed]"
        return "Combat Bridge  [Install Now]"

    def action_toggle_bridge(self, *_):
        if bridge_installer.is_bridge_installed():
            bridge_installer.uninstall_bridge()
        else:
            bridge_installer.install_bridge()
        if self.icon is not None:
            with contextlib.suppress(Exception):
                self.icon.update_menu()

    def menu_startup_toggle(self, *_) -> str:
        if startup.is_startup_enabled():
            return "Start with Windows  [X]"
        return "Start with Windows  [ ]"

    def action_toggle_startup(self, *_):
        startup.toggle_startup()
        if self.icon is not None:
            with contextlib.suppress(Exception):
                self.icon.update_menu()

    def menu_update_status(self, *_) -> str:
        return self.updater.get_status_text()

    def action_update_click(self, *_):
        if self.updater.state == UpdateState.AVAILABLE:
            self.updater.install_in_background(on_exit_callback=self.cleanup_for_update)
        elif self.updater.state in (UpdateState.IDLE, UpdateState.UP_TO_DATE, UpdateState.ERROR):
            self.updater.check_in_background(
                delay_seconds=0.0, on_complete=self.on_update_check_complete
            )

    def action_exit(self, *_):
        self.running = False
        self.rpc_manager.disconnect()
        if self.icon is not None:
            self.icon.stop()

    def create_menu(self) -> Menu:
        return Menu(
            MenuItem("Clair Obscur: Expedition 33 RPC", lambda *_: None, enabled=False),
            Menu.SEPARATOR,
            MenuItem(self.menu_game_status, lambda *_: None, enabled=False),
            MenuItem(self.menu_zone_status, lambda *_: None, enabled=False),
            MenuItem(self.menu_combat_status, lambda *_: None, enabled=False),
            MenuItem(self.menu_discord_status, lambda *_: None, enabled=False),
            Menu.SEPARATOR,
            MenuItem(self.menu_anti_spoiler_toggle, self.action_toggle_anti_spoiler),
            MenuItem(self.menu_bridge_toggle, self.action_toggle_bridge),
            MenuItem(self.menu_startup_toggle, self.action_toggle_startup),
            MenuItem(self.menu_update_status, self.action_update_click),
            Menu.SEPARATOR,
            MenuItem("Exit", self.action_exit),
        )

    def run(self):
        self.worker_thread = threading.Thread(target=self.update_loop, daemon=True)
        self.worker_thread.start()

        # Background update check (delayed 3 seconds to prioritize initial game detection)
        self.updater.check_in_background(
            delay_seconds=3.0,
            on_complete=self.on_update_check_complete,
        )

        img = self.get_icon_image()
        self.icon = pystray.Icon(
            name="Expedition33RPC",
            icon=img,
            title="Expedition 33 Discord RPC",
            menu=self.create_menu(),
        )

        # Hook Win32 notification to guarantee menu items refresh dynamically on every right click
        icon_obj: Any = self.icon
        orig_on_notify = getattr(icon_obj, "_on_notify", None)
        if callable(orig_on_notify):

            def hooked_on_notify(wparam, lparam):
                # 517 is WM_RBUTTONUP (right mouse button released on tray icon)
                if lparam == 517 and self.icon is not None:
                    with contextlib.suppress(Exception):
                        self.icon.update_menu()
                return orig_on_notify(wparam, lparam)

            icon_obj._on_notify = hooked_on_notify

        if self.icon is not None:
            self.icon.run()
